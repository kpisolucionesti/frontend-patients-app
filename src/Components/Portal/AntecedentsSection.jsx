...
import { Box, IconButton, Paper, Tooltip, Typography, Dialog, DialogTitle, DialogContent, DialogActions, Button, TextField } from '@mui/material';
...
const EMPTY = { condition_type: '', description: '', diagnosed_at: null, medication: '', notes: '' };
...
const AntecedentsSection = ({ patientId, readOnly }) => {
  ...
  const handleOpenEdit = (a) => { setEditing(a); setForm({ condition_type: a.condition_type, description: a.description || '', diagnosed_at: a.diagnosed_at || null, medication: a.medication || '', notes: a.notes || '' }); setDialogOpen(true); };
    ...
    <Paper sx={{ p: 1.5 }}>
        ...
        <Box>
          {antecedents.map((a) => (
            <Box key={a.id} sx={{ display: 'flex', alignItems: 'flex-start', gap: 0.25, mb: 0.5, p: 0.5, bgcolor: '#fafafa', borderRadius: 1 }}>
              <Box sx={{ flex: 1 }}>
                <Typography variant="caption" fontWeight={600} sx={{ fontSize: '0.7rem' }}>{a.condition_type}</Typography>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          ...
          <Button size="small" variant="contained" onClick={handleSave} disabled={saving || !form.condition_type}>{saving ? 'Guardando...' : 'Guardar'}</Button>
        </DialogActions>
      </Dialog>
    </Paper>
  );
