    ...
    <Paper sx={{ p: 1.5, borderLeft: '4px solid #e65100', display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1.5 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
          ...
          <Typography variant="caption" fontWeight={600} sx={{ color: '#e65100', fontSize: '0.8rem' }}>
            CIRUGÍAS
          </Typography>
          {scheduledCount > 0 && (
            <Chip label={`${scheduledCount} programada(s)`} size="small" color="warning" sx={{ height: 20, fontSize: '0.6rem' }} />
          )}
        </Box>
        {canEdit && (
                ...
                <TableRow key={s.id} hover>
                  <TableCell>
                    <Chip label={SURGERY_TYPES.find((t) => t.key === s.surgery_type)?.label || s.surgery_type}
                      size="small" sx={{ height: 20, fontSize: '0.6rem', bgcolor: '#e65100', color: 'white' }} />
                  </TableCell>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle sx={{ fontSize: '0.85rem', bgcolor: '#e65100', color: 'white' }}>
