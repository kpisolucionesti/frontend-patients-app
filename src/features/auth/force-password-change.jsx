      ...
      <Card sx={{ maxWidth: 420, width: '100%', mx: 2 }}>
        <CardContent sx={{ p: 4 }}>
          ...
          <form onSubmit={handleSubmit}>
              ...
              <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.75, mb: 2 }}>
                {pwdChecks.map((rule) => (
                  <Typography
                    key={rule.key}
                    variant="caption"
                    sx={{ color: rule.met ? theme.palette.success.main : theme.palette.grey[500], fontWeight: rule.met ? 600 : 400 }}
                  >
                    {rule.met ? '✓' : '✗'} {rule.label}
                  </Typography>
                ))}
            ...
            />
            <Button
              fullWidth variant="contained" type="submit" disabled={saving}
            >
