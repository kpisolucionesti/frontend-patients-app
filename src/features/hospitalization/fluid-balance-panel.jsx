...
const FluidBalancePanel = ({ hospitalizationId, readOnly = false }) => {
    ...
    <Paper sx={{ p: 1.5, borderLeft: 3, borderColor: 'primary.main' }}>
      ...
      {summary && (
        <Grid container spacing={2} sx={{ mb: 2 }}>
          ...
          <Grid item xs={4}>
            <Card sx={{ bgcolor: (summary.total_net || 0) >= 0 ? 'primary.light' : 'warning.light', boxShadow: 3, borderRadius: 1 }}>
              <CardContent sx={{ py: 1, textAlign: 'center' }}>
                <Typography variant="caption" color="text.secondary">Balance Neto</Typography>
                <Typography fontWeight={700} color={summary.total_net >= 0 ? 'primary.main' : 'warning.main'} sx={{ fontSize: '1rem' }}>
      ...
      <TableContainer component={Paper} sx={{ maxHeight: 350, overflow: 'auto', borderRadius: 1 }}>
        <Table size="small" stickyHeader>
          <TableHead>
            <TableRow>
              ...
              {canNurse && !readOnly && <TableCell sx={{ bgcolor: 'primary.main', color: 'white', fontWeight: 600, fontSize: '0.75rem' }}>Acción</TableCell>}
            </TableRow>
          </TableHead>
          <TableBody>
            {records.length === 0 ? (
              <TableRow>
                <TableCell colSpan={canNurse && !readOnly ? 6 : 5} align="center" sx={{ py: 3, color: 'text.secondary' }}>
              ...
              <TableRow
                key={r.id}
                sx={{ bgcolor: r.balance_type === 'intake' ? 'success.light' : 'error.light' }}
              >
                <TableCell>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogActions>
          ...
          <Button variant="outlined" color="success" onClick={handleCreate} disabled={saving || !form.amount} sx={{ fontSize: '0.7rem' }}>
